...
export async function publishToRemoteConfigAction(
  ...
  try {
    console.log("publishToRemoteConfigAction: Building component tree for Remote Config...");
    ...
    if (typeof error === 'object' && error !== null) {
        const firebaseError = error as any; // Cast to any to check for common Firebase error properties
        if (firebaseError.errorInfo && firebaseError.errorInfo.code) {
            ...
            if (firebaseError.errorInfo.code === 'remoteconfig/template-version-mismatch') {
                message = 'Remote Config template version mismatch. Please try again. This can happen if the template was changed on the server since it was last fetched.';
            } else if (firebaseError.errorInfo.code === 'permission-denied' || firebaseError.code === 'PERMISSION_DENIED') {
                message = 'Permission denied. The service account may not have the "Firebase Remote Config Admin" role. Check IAM settings in Google Cloud Console for your service account.';
            }
        } else if (firebaseError.code === 'PERMISSION_DENIED') { // Sometimes the code is at the top level
             message = 'Permission denied. The service account may not have the "Firebase Remote Config Admin" role. Check IAM settings in Google Cloud Console for your service account.';
        }
    }
