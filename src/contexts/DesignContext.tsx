...
export const DesignProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
    ...
    const loadInitialData = async () => {
      ...
      try {
        ...
        templatesSnapshot.forEach((docSnap) => {
          ...
          // Ensure data.templateId which contains "custom/" is read
      ...
      try {
        ...
        layoutsSnapshot.forEach((docSnap) => {
          const data = docSnap.data();
           // Ensure layoutId is read (it should be the docSnap.id if generated by previous logic)
          if (data.layoutId && data.name && data.components && typeof data.nextId === 'number') {
            layouts.push({
              firestoreId: docSnap.id, // Use docSnap.id as the firestoreId
              layoutId: data.layoutId as string, // The layoutId stored in the document
              name: data.name as string,
  ...
  const saveSelectedAsCustomTemplate = useCallback(async (name: string) => {
    ...
    try {
      ...
      toast({ title: "Save Failed (Firestore Sync)", description: `${detail}. Saved locally.`, variant: "destructive" });
      // State is already updated locally optimistically.
    }
  ...
  const deleteCustomComponentTemplate = useCallback(async (appTemplateId: string, firestoreDocId?: string) => {
    ...
    toast({ title: "Custom Template Deleted", description: `Template "${templateName}" removed locally.` });
  ...
  const renameCustomComponentTemplate = useCallback(async (appTemplateId: string, newName: string, firestoreDocId?: string) => {
    ...
    toast({ title: "Custom Template Renamed", description: `Template renamed to "${newName}" locally.` });
  ...
  const saveCurrentCanvasAsLayout = useCallback(async (name: string) => {
    ...
    toast({ title: "Layout Saved", description: `Layout "${name}" saved locally.` });
    ...
    try {
      ...
      toast({ title: "Save Layout Failed (Firestore Sync)", description: `${detail}. Saved locally.`, variant: "destructive" });
    }
  ...
  const deleteSavedLayout = useCallback(async (layoutId: string, firestoreDocId?: string) => {
    ...
    toast({ title: "Layout Deleted", description: `Layout "${layoutName}" removed locally.` });
  ...
  const renameSavedLayout = useCallback(async (layoutId: string, newName: string, firestoreDocId?: string) => {
    ...
    toast({ title: "Layout Renamed", description: `Layout renamed to "${newName}" locally.` });
